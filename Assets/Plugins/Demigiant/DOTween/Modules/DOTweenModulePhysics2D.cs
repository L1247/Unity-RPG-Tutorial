...
namespace DG.Tweening
{
    ...
    #region Public Methods
        ...
        ///     <para>IMPORTANT: a rigidbody2D can't be animated in a jump arc using MovePosition, so the tween will directly set the position</para>
        /// </summary>
        ...
        public static Sequence DOJump(
                this Rigidbody2D target , Vector2 endValue , float jumpPower , int numJumps , float duration , bool snapping = false)
        {
                ...
                )
        {
            ...
            var len                                   = path.Length;
            var path3D                                = new Vector3[len];
            for (var i = 0 ; i < len ; ++i) path3D[i] = path[i];
            ...
            var t = DOTween.To(PathPlugin.Get() , () => trans.localPosition ,
                               ...
                               new Path(pathType , path3D , resolution , gizmoColor) , duration)
                           .SetTarget(target)
        ...
        public static TweenerCore<Vector2 , Vector2 , VectorOptions> DOMove(
                this Rigidbody2D target , Vector2 endValue , float duration , bool snapping = false)
        {
        ...
        public static TweenerCore<Vector2 , Vector2 , VectorOptions> DOMoveX(
                this Rigidbody2D target , float endValue , float duration , bool snapping = false)
        {
                ...
                )
        {
            ...
            var path3D                                = new Vector3[len];
            for (var i = 0 ; i < len ; ++i) path3D[i] = path[i];
            var t = DOTween.To(PathPlugin.Get() , () => target.position , x => target.MovePosition(x) ,
                               new Path(pathType , path3D , resolution , gizmoColor) , duration)
                           .SetTarget(target)
                           .SetUpdate(UpdateType.Fixed);
        ...
        {
            var t = DOTween.To(PathPlugin.Get() , () => target.position , x => target.MovePosition(x) , path , duration)
                           .SetTarget(target);
